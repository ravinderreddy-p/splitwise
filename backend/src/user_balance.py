...
class UsersBalanceUpdate(object):
    def __init__(self):
        pass
    ...
    def update_user_balance(expense_paid_by, expense_shared_by, each_person_share_amount):
        ...
        for shared_by_user in expense_shared_by.split(','):
            ...
            exist_users = UsersBalanceUpdate.get_existing_user(expense_paid_by, shared_by_user.strip())
    
            if exist_users is None:
                UsersBalanceUpdate.add_to_user_balance(expense_paid_by, shared_by_user.strip(), each_person_share_amount)
    
            else:
                UsersBalanceUpdate.update_existing_user_balance(exist_users, each_person_share_amount, expense_paid_by)
